...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf, size_t buf_size, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data)
{
    ...
    do
    {
        ...
        if (3 != frame_info.layer)
            break;
        int ret_t = mp3dec_check_vbrtag(hdr, frame_size, &frames, &delay, &padding);
        if (ret_t > 0)
        {
...
int mp3dec_iterate_cb(mp3dec_io_t *io, uint8_t *buf, size_t buf_size, MP3D_ITERATE_CB callback, void *user_data)
{
    ...
    {
        size_t readed_t = io->read(buf + MINIMP3_ID3_DETECT_SIZE, buf_size - MINIMP3_ID3_DETECT_SIZE, io->read_data);
        if (readed_t > (buf_size - MINIMP3_ID3_DETECT_SIZE))
            return MP3D_E_IOERROR;
        filled += readed_t;
    }
    if (filled < MINIMP3_BUF_SIZE)
    ...
    do
    {
        ...
        {   /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
            ...
            filled -= consumed;
            consumed = 0;
            size_t readed_t = io->read(buf + filled, buf_size - filled, io->read_data);
            if (readed_t > (buf_size - filled))
                return MP3D_E_IOERROR;
            if (readed_t != (buf_size - filled))
                eof = 1;
            filled += readed_t;
...
int mp3dec_detect(const char *file_name)
{
    ...
    int ret = mp3dec_open_file(file_name, &map_info);
    if ((ret))
        return ret;
...
int mp3dec_ex_open(mp3dec_ex_t *dec, const char *file_name, int flags)
{
    int ret = mp3dec_open_file(file_name, &dec->file);
    if (!dec)
        return MP3D_E_PARAM;
    if ((ret))
        return ret;
...
{
    mp3dec_map_info_t map_info;
    int ret = mp3dec_open_file_w(file_name, &map_info);
    if ((ret))
        return ret;
    return mp3dec_detect_mapinfo(&map_info);
}
...
int mp3dec_ex_open_w(mp3dec_ex_t *dec, const wchar_t *file_name, int flags)
{
    int ret = mp3dec_open_file_w(file_name, &dec->file);
    if ((ret))
        return ret;
    return mp3dec_ex_open_mapinfo(dec, flags);
}
